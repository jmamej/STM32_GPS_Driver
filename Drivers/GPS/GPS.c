...
int GPS_GetData(void){
	...
	printf("Time: %s\n", GPS_GetCompleteTime(0));
	printf("Date: %s\n", GPS_GetCompleteDate());
	printf("Location: %s\n", GPS_GetCompleteLocation());
	printf("SV: %d\n", GPS_GetSatellitesVisible());
	printf("SU: %d\n", GPS_GetSatellitesInUse());
#endif
...
int GPS_IsDataReady(){
	...
	if(GPS_data_ready_flag) {
		...
		if(current_time >= (rx_data_timer + 5) && rxBuffer[rxBufferIndex-1] == 0x0A && rxBuffer[rxBufferIndex-2] == 0x0D) {
			return 1;
		}
	}
	return 0;
